...
export function MessageList() {
  ...
  const questions = useMemo(() => visible.filter((m) => m.role === 'user'), [visible]);
  ...
  const jumpToMessage = useCallback((id: string) => {
    stick.current = false;
    // An early click can target history that the background transition has not mounted yet.
    flushSync(() => setBudget(Number.MAX_SAFE_INTEGER));
    const target = anchors.current.get(id);
    if (!target || !ref.current) return;
    target.scrollIntoView({ block: 'start', behavior: 'instant' });
    lastScrollTop.current = Math.max(0, ref.current.scrollTop);
  }, []);
  ...
  return (
    <div className="message-list-shell relative flex min-h-0 flex-1">
      <div ref={ref} data-message-scroller className="min-w-0 flex-1 overflow-y-auto overscroll-y-none px-4 py-8">
        <div ref={inner} className="mx-auto flex max-w-3xl flex-col gap-7">
          ...
          {rendered.map((turn) => {
            ...
            return (
              <div key={first.id} className={cn(!active && 'msg-item', first.role === 'user' && 'message-anchor')}
                data-user-message-id={first.role === 'user' ? first.id : undefined}
                ref={first.role === 'user' ? (node) => { if (node) anchors.current.set(first.id, node); else anchors.current.delete(first.id); } : undefined}>
      ...
      <MessageNavigation key={currentId ?? 'draft'} messages={questions} scrollRef={ref} onJump={jumpToMessage} />
    </div>
  );
}
